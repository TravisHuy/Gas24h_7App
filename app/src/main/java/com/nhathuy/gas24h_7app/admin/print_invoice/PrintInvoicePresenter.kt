...
class PrintInvoicePresenter @Inject constructor(private val orderRepository: OrderRepository,
                                                private val productRepository: ProductRepository,
                                                private val userRepository: UserRepository
) : PrintInvoiceContract.Presenter{

    private var view:PrintInvoiceContract.View? = null
    ...
    override fun attachView(view: PrintInvoiceContract.View) {
        this.view = view
    }
    ...
    override fun getSelectedOrders(): List<Order> {
        return allOrders.filter { it.id in selectedOrders }
    }
        ...
        coroutineScope.launch {
            ...
            try {
                selectedOrderIds.map {
                        orderId -> orderRepository.updateOrderStatus(orderId, OrderStatus.SHIPPED)
                ...
                view?.updateOrderList(allOrders,selectedOrders)
                view?.showMessage("Selected orders are being delivered")
            }
            catch (e:Exception){
