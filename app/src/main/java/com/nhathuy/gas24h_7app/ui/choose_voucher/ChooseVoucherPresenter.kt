...
class   ChooseVoucherPresenter @Inject constructor(
    ...
    override fun searchVouchers(query: String) {
        val filteredVouchers = allVouchers.filter { voucher ->
            voucher.id.contains(query, ignoreCase = true) || voucher.code?.contains(
                query,
                ignoreCase = true
            ) == true
        }
    ...
    override fun onAgreeButtonClick() {
        val selectedVoucher = selectedVoucherId?.let { voucherId ->
            allVouchers.find {
                it.id == voucherId
            }
        }
